...
contract Sequencer is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function createSequencer(
        ...
        require(
            _mintAddress != address(0),
            "Invild address, address can not be 0"
        );
        // check already have sequencer
        require(
            sequencers[msg.sender].mintAddress == address(0),
            "Already has been created"
        );
        require(
            rel[_mintAddress] == address(0),
            "This mint address already has owner"
        );
     ...
     */
    function getSequencer(address signer)
        public
        view
    ...
    function deleteSequencer(address signer) internal {
        ...
        emit SequencerDelete(
            sequencers[signer].mintAddress,
            sequencers[signer].nodeID
        );
        // change index
